...
export default function App() {
  ...
  const setTab = t => { localStorage.setItem("fr:tab", t); setTabRaw(t); };
  ...
  useEffect(() => {
    (async () => {
      try {
        let [fs, evts, acts] = await Promise.all([api.getFriends(), api.getEvents(), api.getActivities()]);
        if (fs.length === 0) {
          await Promise.all(SEED_FRIENDS.map(f => api.upsertFriend(f)));
          fs = await api.getFriends();
        }
        setFriends(fs);
        setEvents(evts);
        setActivities(acts);
  ...
  const saveFriend = useCallback((data) => {
    const isNew  = !data.id;
    const friend = isNew ? { ...EMPTY_FRIEND, ...data, id: `u-${Date.now()}` } : data;
    setFriends(prev => isNew ? [...prev, friend] : prev.map(f => f.id === friend.id ? friend : f));
  ...
  const addEvent = useCallback((evt) => {
    ...
    api.createEvent(evt).then(result => {
      if (result?.event) {
  ...
  const addActivity = useCallback((label) => {
    api.createActivity({ label }).then(act => {
  ...
  const advanceCascade = useCallback((eventId) => {
    setEvents(prev => prev.map(e => {
      ...
      return { ...e, invites: e.invites.map(i => i.friendId === nextQueued.friendId ? { ...i, inviteStatus: "invited" } : i) };
    }));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f7f7f5", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ background: "#fff", borderBottom: "1px solid #f0f0ee", paddingTop: 20, paddingLeft: 16, paddingRight: 16 }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          ...
          {noEmail > 0 && (
            <div style={{ marginBottom: 12, padding: "8px 12px", background: "#fff7ed", borderRadius: 10, border: "1px solid #fed7aa", fontSize: 12, color: "#9a3412" }}>
              {noEmail} friends missing emails — add them to enable calendar invites
            </div>
          )}
          <div style={{ display: "flex", borderTop: "1px solid #f3f4f6" }}>
            {TABS.map(t => (
              <button key={t.id} onClick={() => { setTab(t.id); if (t.id !== "add") closeEditor(); }} style={{
                flex: 1, paddingTop: 11, paddingBottom: 11, fontSize: 12, fontWeight: 600,
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "20px 16px" }}>
        {tab === "plan" && (
          <PlanTab friends={friends} events={events} activities={activities} onCreate={addEvent} onAddActivity={addActivity} goToEvents={() => setTab("events")} />
        )}
        {tab === "events" && (
