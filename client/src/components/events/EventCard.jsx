...
export function EventCard({
  ...
  const setResponse = (friendId, resp) => {
    onUpdate(event.id, (e) => ({
      ...e,
      invites: e.invites.map((i) => (i.friendId === friendId ? { ...i, response: resp } : i)),
    }));
    setExpanded(null);
  };
  ...
  const handleToggleLeg = (friendId, legId, currentAttendingLegs) => {
    ...
    api
      .updateInviteAttendingLegs(event.id, friendId, next)
      .then((updated) => onUpdate(event.id, () => updated));
  };

  const startFinalize = () => {
    const init = {};
    yesInv.forEach((i) => {
      init[i.friendId] = i.showed !== false;
    });
    setTempShowed(init);
  ...
  const saveFinalize = () => {
    onUpdate(event.id, (e) => ({
      ...
      invites: e.invites.map((i) =>
        i.inviteStatus !== "queued" && i.response === "yes"
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
          ...
          {event.cascade && (
            <Pill
              text={`Cascade ${accepted}/${event.maxCapacity ?? "?"}`}
              bg="#eef2ff"
              c="#4338ca"
            />
          )}
        ...
        <div style={{ marginBottom: 10, paddingLeft: 8, borderLeft: "2px solid #e5e7eb" }}>
          {event.legs.map((leg) => (
            <div key={leg.id} style={{ fontSize: 11, color: "#6b7280", marginBottom: 3 }}>
              ...
              {" — "}
              {leg.label}
              {leg.location ? <span style={{ color: "#9ca3af" }}> @ {leg.location}</span> : null}
            </div>
          ))}
        </div>
      )}
            ...
            <div key={inv.friendId}>
              ...
              >
                <div>
                  ...
                  {email && (
                    <span style={{ fontSize: 10, color: "#9ca3af", marginLeft: 6 }}>{email}</span>
                  )}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  {event.finalized && inv.response === "yes" && (
                    <span
                      style={{
                        fontSize: 11,
                        fontWeight: 700,
                        color: inv.showed ? "#16a34a" : "#dc2626",
                      }}
                    >
                  ...
                  {!event.finalized && (
                    <button
                      onClick={() => setExpanded(isExp ? null : inv.friendId)}
                      style={{
                        fontSize: 10,
                        padding: "2px 6px",
                        borderRadius: 5,
                ...
                >
                  ...
                  {["yes", "maybe", "no"].map((r) => {
                    ...
                    return (
                      <button
                        key={r}
                        onClick={() => handleManualResponse(inv.friendId, r)}
                        style={{
                          padding: "3px 9px",
                          borderRadius: 8,
                          fontSize: 11,
                ...
                >
                  {["yes", "maybe", "no", "ghosted"].map((r) => {
                    ...
                    return (
                      <button
                        key={r}
                        onClick={() => setResponse(inv.friendId, r)}
                        style={{
                          padding: "3px 9px",
                          borderRadius: 8,
                          fontSize: 11,
                ...
                >
                  <span style={{ fontSize: 10, color: "#9ca3af" }}>
                    {!inv.attendingLegs
                      ? "All segments"
                      : inv.attendingLegs
                          .map((id) => event.legs.find((l) => l.id === id)?.label ?? id)
                          .join(", ")}
                  </span>
                  {!event.finalized &&
                    event.legs.map((leg) => {
                      ...
                      return (
                        <button
                          key={leg.id}
                          onClick={() => handleToggleLeg(inv.friendId, leg.id, inv.attendingLegs)}
                          style={{
                            padding: "2px 7px",
                            borderRadius: 99,
                            fontSize: 10,
      ...
      {event.cascade && queued.length > 0 && !event.finalized && (
        ...
        >
          ...
          {queued.slice(0, 4).map((inv, i) => (
            ...
            >
              <span style={{ fontSize: 12, color: "#9ca3af" }}>
                #{invited.length + i + 1} — {getName(inv.friendId)}
              </span>
              {getEmail(inv.friendId) && (
                <span style={{ fontSize: 10, color: "#c7d2fe" }}>{getEmail(inv.friendId)}</span>
              )}
            </div>
          ))}
          {queued.length > 4 && (
            <div style={{ fontSize: 11, color: "#9ca3af" }}>+{queued.length - 4} more in queue</div>
          )}
          <button
            onClick={() => onAdvanceCascade(event.id)}
            style={{
              marginTop: 6,
              width: "100%",
              padding: "6px",
          ...
          >
            Invite next → {getName(nextQueued.friendId)}
          </button>
        </div>
      )}
                  ...
                  return (
                    ...
                    >
                      <span style={{ fontSize: 13, color: "#374151" }}>
                        {getName(inv.friendId)}
                      </span>
                      <button
                        onClick={() => setTempShowed((p) => ({ ...p, [inv.friendId]: !showed }))}
                        style={{
                          padding: "4px 12px",
                          borderRadius: 8,
                          fontSize: 11,
                          fontWeight: 700,
                          background: showed ? "#dcfce7" : "#fee2e2",
                          color: showed ? "#15803d" : "#b91c1c",
                          border: "none",
                          cursor: "pointer",
                        }}
                      >
                        {showed ? "✓ Showed" : "✗ Flaked"}
                      </button>
                    </div>
                  );
                })}
