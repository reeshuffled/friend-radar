...
export function CalendarAuditPanel({ audit, friends, onConfirmHang, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 12, fontWeight: 700, color: "#111827", letterSpacing: -0.2 }}>
          Calendar Audit
        </span>
      ...
      <div style={{ maxHeight: 340, overflowY: "auto" }}>
        {tab === "matched" &&
          (matchedCount === 0 ? (
            <p style={{ margin: 0, padding: "12px 14px", fontSize: 12, color: "#9ca3af" }}>
              No matched events.
            </p>
          ) : (
              ...
              return (
                <div key={key} style={{ padding: "8px 14px", borderBottom: "1px solid #f9fafb" }}>
                  <div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 3 }}>
                    {ev.date}
                    {ev.title && (
                      <span style={{ color: "#374151", fontWeight: 500 }}> · {ev.title}</span>
                    )}
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {ev.friendIds.map((id) => {
                      ...
                      return (
                        f && (
                          <span key={id} style={chip(f.name, "#dcfce7", "#15803d")}>
                            ✓ {f.name}
                          </span>
                        )
                      );
        ...
        {tab === "unmatched" &&
          (unmatchedCount === 0 ? (
            <p style={{ margin: 0, padding: "12px 14px", fontSize: 12, color: "#9ca3af" }}>
              No unmatched events — great coverage!
            </p>
          ) : (
            audit.unmatchedEvents.map((ev) => {
              ...
              const fuzzy = (ev.fuzzyMatches ?? []).filter(
                (id) => !conf.includes(id) && !dism.has(id)
              );
                    ...
                    {fuzzy.map((id) => {
                      ...
                      return (
                        f && (
                          <span
                            key={id}
                            style={chip(f.name, "#fef9c3", "#92400e", "1px solid #fde68a")}
                          >
                            {f.name}?
                            <button
