...
export function RankSession({ orderedIds, newId, prompt, friends, onComplete, onCancel }) {
  ...
  const rowStyle = (isNew, isDragging) => ({
    ...
    opacity:
      dragFrom !== null && dragFrom === friends.findIndex((f) => f.id === activeOrder[0]) ? 0.5 : 1,
  });
    ...
    <div style={overlay} onClick={(e) => e.target === e.currentTarget && onCancel()}>
      <div style={panel}>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>
              Pairwise comparison → interpolated 0–10 score
            </div>
          </div>
          <button
            onClick={onCancel}
        ...
        {!isDone && !cmp && !session.done && (
          <div
            style={{ textAlign: "center", padding: "12px 0 20px", fontSize: 13, color: "#6b7280" }}
          >
              ...
              >
                ...
                <span
                  style={{
                    ...
                    color: isNew ? "#1d4ed8" : "#374151",
                  }}
                >
                  {fr?.name ?? id}
                  {isNew && (
                    <span
                      style={{ marginLeft: 6, fontSize: 10, color: "#3b82f6", fontWeight: 700 }}
                    >
                      ← placing
                    </span>
                  )}
                ...
                {fr?.groups?.length > 0 && (
                  <div style={{ display: "flex", gap: 3 }}>
                    {fr.groups.slice(0, 2).map((g) => (
                      <Pill key={g} text={g} bg="#f3f4f6" c="#6b7280" />
                    ))}
                  </div>
                )}
                {isDone && rating != null && (
                ...
                {isDone && (
                  <span
                    style={{ color: "#d1d5db", fontSize: 14, flexShrink: 0, userSelect: "none" }}
                  >
                    ⣿
                  </span>
                )}
