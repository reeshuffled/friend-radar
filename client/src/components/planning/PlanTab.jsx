...
export function PlanTab({ friends, events, activities = [], onCreate, onAddActivity, goToEvents }) {
  ...
  useEffect(() => {
    ...
    const eStart = quickMode
      ? (() => {
          const now = new Date();
          return `${String(now.getHours()).padStart(2, "0")}:${now.getMinutes() >= 30 ? "30" : "00"}`;
        })()
      : startTime;
    let cancelled = false;
    api
      .checkFreeBusy(eDate, eStart, endTime, emails)
      .then((result) => {
        if (!cancelled) setCalBusyMap(result.friends ?? {});
      })
      .catch(() => {
        if (!cancelled) setCalBusyMap({});
      });
    return () => {
      cancelled = true;
    };
  ...
  const queue = cascade
    ? scored.filter((f) => !f.isBusyThisWeek && !f.inCooldown).slice(0, maxCap * 2)
    : [];
  ...
  const bestActivityForGroup = (groupIds) => {
    ...
    return activities
      .map((a) => ({
        id: a.id,
        avg: grp.reduce((s, f) => s + (f.interests?.[a.id] ?? 1), 0) / grp.length,
      }))
  ...
  const doReroll = (locked = lockedFriends, size = shuffleSize) => {
    ...
    const newIds = [
      ...lockedArr,
      ...pool.slice(0, Math.max(0, size - lockedArr.length)).map((f) => f.id),
    ];
    setShuffleGroup(newIds);
    if (!lockedActivity) setActId(bestActivityForGroup(newIds));
  };

  const toggleLockFriend = (id) =>
    setLockedFriends((prev) => {
  ...
  const createNormal = () => {
    ...
    const effectiveEndFin = effectiveLegs
      ? effectiveLegs[effectiveLegs.length - 1].endTime
      : endTime;
    onCreate({
      id: `evt-${Date.now()}`,
      activityId: actId,
      date: effectiveDate,
      startTime: effectiveStartFin,
      endTime: effectiveEndFin,
      location,
      legs: effectiveLegs,
      invites: inviteIds.map((id) => {
        ...
        return {
          ...
          inviteChannel: friend?.preferredChannel ?? "email",
          attendingLegs: null,
        };
      }),
      notes: "",
  ...
  return (
    <div>
            ...
            <button
              ...
              onClick={() => {
                ...
                if (m.id === "shuffle") {
                  ...
                  setCascade(false);
                  setShuffleMode(true);
                  doReroll(lockedFriends, shuffleSize);
                }
              }}
              style={{
                flex: 1,
      ...
      {!quickMode && cascade && (
        ...
        >
          <div style={{ flex: 1, fontSize: 11, color: "#0369a1" }}>
            Invite #1 now → queue the rest → auto-advance every 36hrs
          </div>
      ...
      {quickMode && (
        ...
        >
          <div style={{ fontSize: 11, color: "#0369a1", fontWeight: 700, marginBottom: 7 }}>
            Short notice — planners will score lower. Pick what fits your energy.
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 12, color: "#0369a1", fontWeight: 600, whiteSpace: "nowrap" }}>
              My energy:
            </span>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: actId === a.id ? "1.5px solid #4f46e5" : "1px solid #e5e7eb",
                  cursor: "pointer",
                }}
              >
                {a.label}
                {actId === a.id && (
                  <span style={{ fontSize: 9, marginLeft: 4, opacity: 0.8 }}>({costLabel})</span>
                )}
              </button>
            );
          })}
          {addingActivity ? (
            ...
            >
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Escape" && (setAddingActivity(false), setNewActLabel(""))
                }
          ...
          <div style={{ marginTop: 5, fontSize: 11, color: "#9ca3af" }}>
            ...
            <span
              style={{
                fontWeight: 700,
                color:
                  (activities.find((a) => a.id === actId)?.energyCost ??
                    SOCIAL_ENERGY_COSTS[actId] ??
                    0) <= 0.25
                    ? "#16a34a"
                    : (activities.find((a) => a.id === actId)?.energyCost ??
                          SOCIAL_ENERGY_COSTS[actId] ??
        ...
        <>
          ...
          <div style={{ marginBottom: 6 }}>
            <button
              ...
              style={{
                fontSize: 11,
                color: legs.length > 0 ? "#4f46e5" : "#9ca3af",
                background: "none",
                border: "none",
                cursor: "pointer",
                padding: "4px 0",
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      <input
                        ...
                        onChange={(e) =>
                          setLegs((ls) =>
                            ls.map((l, j) => (j === i ? { ...l, startTime: e.target.value } : l))
                          )
                        }
                        style={{
                          flex: 1,
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setLegs((ls) =>
                            ls.map((l, j) => (j === i ? { ...l, endTime: e.target.value } : l))
                          )
                        }
                        style={{
                          flex: 1,
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setLegs((ls) =>
                            ls.map((l, j) => (j === i ? { ...l, location: e.target.value } : l))
                          )
                        }
                  ...
                  onClick={() =>
                    setLegs((ls) => [
                      ...ls,
                      {
                        id: `leg-${Date.now()}`,
                        label: "",
                        startTime: legs[legs.length - 1]?.endTime ?? endTime,
                        endTime,
                        location: "",
                      },
                    ])
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 13 }}>
        ...
        >
          ...
          <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 1 }}>
            Solo anchor — removes coordination pressure.
          </div>
        </button>
        <button
        ...
        >
          <div
            style={{ fontSize: 12, fontWeight: 700, color: plusOneAllowed ? "#6d28d9" : "#374151" }}
          >
            {plusOneAllowed ? "✓ " : ""}+1 welcome
          </div>
          <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 1 }}>
            Boosts Willing for people who need an anchor.
          </div>
        </button>
      </div>
      ...
      {shuffleMode && (
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", width: 60 }}>
              Activity
            </span>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#111827" }}>
              {activities.find((a) => a.id === actId)?.label ?? actId}
            </span>
            <button
              onClick={() => setLockedActivity((v) => !v)}
              style={{
                fontSize: 12,
                background: "none",
                border: "none",
              ...
              title={lockedActivity ? "Unlock activity" : "Lock activity"}
            >
              {lockedActivity ? "🔒" : "🔓"}
            </button>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            ...
            {shuffleGroup.length === 0 && (
              <span style={{ fontSize: 11, color: "#9ca3af" }}>Hit Re-roll to pick a group</span>
            )}
            {shuffleGroup.map((id) => {
              ...
              return (
                ...
                >
                  {f.name}
                  <button
                    onClick={() => toggleLockFriend(id)}
                    style={{
                      fontSize: 11,
                      background: "none",
                      border: "none",
                      cursor: "pointer",
                      color: locked ? "#4f46e5" : "#d1d5db",
                      padding: 0,
                      lineHeight: 1,
                    }}
      ...
      <div style={{ marginBottom: 13 }}>
        ...
        <div
          style={{ display: "flex", gap: 0, background: "#f3f4f6", borderRadius: 12, padding: 4 }}
        >
          {[
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        {scored.map((f, idx) => {
          ...
          const isSelected =
            !cascade && (shuffleMode ? shuffleGroup.includes(f.id) : selected.has(f.id));
          ...
          return (
            <div
              ...
              style={{
                ...
                padding: "10px 12px",
                opacity: cardOpacity,
                cursor: cascade ? "default" : f.isBusyThisWeek ? "default" : "pointer",
              }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {cascade ? (
                  ...
                  >
                    {isInCascade ? (queuePos === 1 ? "→" : queuePos) : "·"}
                  </div>
                ) : (
                  <div
                    style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                    ...
                    {cascade && isInCascade && (
                      <Pill
                        text={queuePos === 1 ? "Invite first" : `Queue #${queuePos}`}
                        bg={queuePos === 1 ? "#dcfce7" : "#eef2ff"}
                        c={queuePos === 1 ? "#15803d" : "#4338ca"}
                      />
                    )}
                    {f.calBusy === true && <Pill text="Busy (cal)" bg="#fff7ed" c="#c2410c" />}
                    {!f.slotMatch && f.availSlots?.length > 0 && f.calBusy == null && (
                      <Pill text="May not be free" bg="#fff7ed" c="#c2410c" />
                    )}
                    ...
                    {f.inCooldown && !f.isBusyThisWeek && (
                      <Pill text={`${f.daysUntilDue}d to go`} bg="#eff6ff" c="#2563eb" />
                    )}
                    {st?.flakeRate != null && st.flakeRate > 0.4 && (
                      <Pill
                        text={`${Math.round(st.flakeRate * 100)}% flake`}
                        bg="#fee2e2"
                        c="#b91c1c"
                      />
                    )}
                    {plusOneAllowed && f.comfortLvl === "needs-plus1" && (
                      <Pill text="+1 ↑ likely" bg="#f5f3ff" c="#6d28d9" />
                    )}
                    {plusOneAllowed && f.comfortLvl === "familiar" && (
                      <Pill text="+1 ↑ helps" bg="#f0fdf4" c="#15803d" />
                    )}
                    {!plusOneAllowed && f.comfortLvl === "needs-plus1" && (
                      <Pill text="wants +1" bg="#fef3c7" c="#92400e" />
                    )}
                    ...
                    {bestSynergy && !conflictNames.length && (
                      <Pill
                        text={`Good w/ ${bestSynergy.name} (${bestSynergy.syn.score}★)`}
                        bg="#dcfce7"
                        c="#15803d"
                      />
                    )}
                  ...
                  {f.email && (
                    <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 2 }}>{f.email}</div>
                  )}
                </div>
                ...
                <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 8 }}>
                  <WATBars
                    ...
                    able={f.able}
                    trust={f.trust}
                    slotMatch={f.slotMatch}
                    inCooldown={f.inCooldown}
                    energyMod={f.energyMod}
                  />
                  <ScoreDisplay
                    score={f.score}
                    willing={f.willing}
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            {!message.trim() && (
              <div style={{ fontSize: 11, color: "#f59e0b", marginTop: 3 }}>
                Write a message — nothing gets sent without one
              </div>
            )}
          </div>
          ...
          {cascade ? (
            <>
              <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 8, textAlign: "center" }}>
                Will invite {queue[0]?.name} first · {queue.length - 1} queued · target {maxCap}{" "}
                total
              </div>
              {queue.some((a, i) =>
                queue.some(
              ...
              >
                Start cascade — invite {queue[0]?.name} now →
              </button>
            </>
          ) : (
            <button
            ...
            >
              Create "{actLabel}" hang with {selected.size}{" "}
