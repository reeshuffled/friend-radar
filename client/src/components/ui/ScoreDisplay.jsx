...
export function ScoreDisplay({
  ...
  const wC = Math.round(willing * 0.4);
  const aC = Math.round(able * 0.35);
  const tC = Math.round(trust * 0.25);
  const base = wC + aC + tC;
  const color = score >= 75 ? "#16a34a" : score >= 55 ? "#d97706" : "#dc2626";

  const modifier = isBusyThisWeek
    ? { label: "Busy penalty ×0.15", color: "#f87171" }
    : inCooldown
      ? { label: `Cooldown (${daysUntilDue}d left)`, color: "#60a5fa" }
      : targetFreqDays && ds !== null && ds >= targetFreqDays
        ? { label: `Overdue boost (+${score - base})`, color: "#fbbf24" }
        : ds === null && targetFreqDays
          ? { label: "Never hung boost", color: "#c084fc" }
          : null;
    ...
    >
      {show && (
        ...
        >
          ...
          {[
            ["W", willing, "40%", wC, "#a78bfa"],
            ["A", able, "35%", aC, "#60a5fa"],
            ["T", trust, "25%", tC, "#34d399"],
          ].map(([k, v, w, c, col]) => (
          ...
          <div style={{ borderTop: "1px solid #374151", marginTop: 7, paddingTop: 7 }}>
            ...
            >
              <span>Base</span>
              <span style={{ fontWeight: 600, color: "#d1d5db" }}>{base}</span>
            </div>
            {modifier && (
              <div
                style={{ display: "flex", justifyContent: "space-between", gap: 20, marginTop: 2 }}
              >
