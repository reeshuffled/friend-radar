...
export function scoreFor(friend, actId, eventSlot, events, allowsPlusOne = false, eventDate = null, activities = [], venueProximity = "mine") {
  ...
  const drainRate     = { low: 0.10, medium: 0.25, high: 0.42 }[friend.workDrain ?? "medium"] ?? 0.25;
  const socialMod     = { introvert: -0.12, ambivert: 0, extrovert: 0.10 }[friend.socialType ?? "ambivert"] ?? 0;
  ...
  if (eventDate) {
    ...
    const needsDays  = NOTICE_PREFS.find(n => n.id === (friend.noticePreference ?? "few-days"))?.days ?? 3;
  ...
  let trust;
  if (st && st.total >= 2) {
    trust = (1 - (st.flakeRate ?? 0)) * 5 + (1 - (st.ghostRate ?? 0)) * 2.5 + (st.yesRate ?? 0.5) * 2.5;
  ...
  const isBusyThisWeek = !!(friend.busyUntil && new Date(friend.busyUntil) >= new Date(new Date().toISOString().split("T")[0]));
...
function computeVelocityBonus(friendId, events) {
  ...
  for (const e of events) {
    for (const inv of e.invites) {
      ...
      const commitment =
        inv.response === "yes"   ? 1.0 :
        inv.response === "maybe" ? 0.5 :
        inv.response === "no"    ? 0.7 : 0;
