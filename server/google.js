...
export async function createCalendarEvent({ event, friendNames, friendEmails }) {
  const client = await getAuthorizedClient();
  const auth   = getAuth();
...
export async function sendGmailMessage({ to, subject, html, text }) {
  const client = await getAuthorizedClient();
  const auth   = getAuth();
