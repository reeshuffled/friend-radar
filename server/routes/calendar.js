...
import {
  getAuth, getAllFriends,
  getCalSyncToken, saveCalSyncToken,
  bulkUpdateLastHangDate, getFriendLastHangDatesByIds,
} from "../db/queries.js";
...
async function syncHangs(syncToken, includeAudit = false) {
  ...
  for (const event of calEvents) {
    ...
    const others = (event.attendees ?? []).filter(a => !a.self && a.responseStatus !== "declined");
...
function requireGoogleAuth(res) {
  ...
  if (!auth?.refresh_token) {
    res.status(401).json({ error: "not_authenticated", message: "Connect Google first — visit /api/auth/google" });
    return false;
  }
  return true;
}
